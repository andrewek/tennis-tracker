...
### Requirement: Match schedule section is displayed with placeholder data

The page SHALL display a match schedule section. Until the real schema is built, placeholder data SHALL be used. Each match entry SHALL show the day of week, date, time, home/away designation with opponent name, and location.

#### Scenario: Match schedule renders placeholder entries
- **WHEN** the team show page loads
- **THEN** the match schedule section is visible
- **THEN** each placeholder match shows day/date/time, opponent (formatted as "HOME v. X" or "AWAY v. X"), and location name

### Requirement: Page is responsive on mobile and desktop

The page layout SHALL be usable on both mobile-sized and desktop-sized viewports. On mobile the sections stack vertically; on desktop they display in a two-column grid.
