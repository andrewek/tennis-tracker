...
The `/g/:group_slug/settings/locations/new` page SHALL provide a form for creating a location with fields: name (required), address (required), google_maps_url (optional). On success, redirect to the index page. On duplicate name, display a validation error.

#### Scenario: Successful location creation
- **WHEN** a group owner submits the create form with a unique name and valid address
...
- **WHEN** a group owner submits the create form without a name or address
- **THEN** a validation error is shown and no location is created

### Requirement: Edit location form allows group owners to update venue details
The `/g/:group_slug/settings/locations/:id/edit` page SHALL provide a pre-populated form for updating a location's name, address, and google_maps_url. On success, redirect to the index page.
