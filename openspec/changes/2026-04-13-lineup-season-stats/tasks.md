...
- [x] 1.1 Add `lib/tennis_tracker/tennis/participation_type.ex` — `use Ash.Type.Enum, values: [:playing, :out, :neutral]`

- [x] 1.2 Update `TeamLineupSlot`:
...
- [x] 1.3 Generate migration: `mix ash_postgres.generate_migrations --name replace_exclusion_slot_with_participation_type`
...
- [x] 1.4 Update `MatchLineupAssignment` — replace all `is_exclusion_slot` checks with `participation_type == :out`:
...
- [x] 1.5 Update `Team` default slot creation — `is_exclusion_slot: true` → `participation_type: :out` (the "Out" slot created automatically on team create, line ~156)

- [x] 1.6 Update `LineupEditLive` — replace `is_exclusion_slot` references in `load_board`:
...
- [x] 1.7 Update `ShowLive` — `Enum.reject(& &1.is_exclusion_slot)` on line 37 → reject where `participation_type == :out`

- [x] 1.8 Update `EditLive` (teams) — slot delete guard on line 682: `slot.is_exclusion_slot` → `slot.participation_type == :out`

- [x] 1.9 Update the slot creation UI in `EditLive` (teams):
...
- [x] 1.10 Write tests for slot management changes:
...
- [x] 1.11 Run `mix precommit` — confirm no compile warnings, tests pass

---

## Phase 2: Season stats domain function

- [x] 2.1 Add `season_stats_for_team!/3` to `TennisTracker.Tennis` domain:
...
- [x] 2.2 Write tests for `season_stats_for_team!`:
...
- [x] 3.1 Add new assigns to `LineupEditLive.mount/3`:
  - `:stats_open` → `false`
  - `:stats_sort` → `:name`
  - `:season_stats` → `nil`
  - `:prev_match` → `nil`
...
- [x] 3.2 Extend `load_board/2` to also:
...
- [x] 3.3 Add `handle_event("toggle_stats", ...)` — flips `:stats_open`

- [x] 3.4 Add `handle_event("set_stats_sort", %{"sort" => sort}, ...)` — updates `:stats_sort`

- [x] 3.5 Add toggle button to the header bar in `render/1`:
...
- [x] 3.6 Add prev/next navigation to the header bar in `render/1`:
...
- [x] 3.7 Implement the `stats_drawer/1` private component:
