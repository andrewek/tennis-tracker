...
The system SHALL expose a `create_match/1` domain function (via `AshPhoenix.Form`) that creates a match associated with a team. Authenticated users SHALL be able to create matches.

#### Scenario: Authenticated user creates a match
- **WHEN** an authenticated user submits a valid match creation form
- **THEN** a new Match record is created in the database associated with the correct team

#### Scenario: Invalid match creation is rejected
- **WHEN** an authenticated user submits a match creation form with missing required fields
...
The system SHALL provide a page at `/matches/:id` that displays all details for a single match: team name, date, time (formatted in the match's local timezone), opponent, home/away status, location name, and location address. If a `google_maps_url` is present on the location, it SHALL be rendered as a clickable link. If no location is set, the location section SHALL show a placeholder (e.g. "Location TBD").

#### Scenario: User views match detail page
- **WHEN** an authenticated user navigates to `/matches/:id`
...
### Requirement: Match detail page links back to the team

The match show page SHALL display the team name as a link back to the team show page (`/teams/:id`).

#### Scenario: User navigates back to team from match page
- **WHEN** a user is on the match show page
- **THEN** a link to the parent team's show page is visible and navigable
