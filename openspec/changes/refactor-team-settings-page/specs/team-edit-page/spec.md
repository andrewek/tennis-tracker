...
**Reason**: URL renamed to `/settings`. The old `/edit` route is removed.
**Migration**: Use `/g/:slug/teams/:id/settings`. All hardcoded `/edit` links in the codebase are updated as part of this change.

### Requirement: Captain can edit an existing slot inline
**Reason**: Replaced by modal-based edit (see "Captain can edit a slot via modal" above). The inline form is removed in favor of a consistent modal experience for both add and edit.
**Migration**: No data migration needed. Captains now click the edit button to open a modal instead of editing inline.
