...
Every LiveView route for Tennis domain data SHALL be nested under `/g/:group_slug/`. The group slug is a human-readable string (e.g., `my-tennis-group`). The previous flat routes (e.g., `/teams`, `/players`) SHALL no longer exist. Navigating to `/g/:group_slug/` SHALL display that group's home page.
...
- **WHEN** an authenticated user without a GroupMembership for "other-group" navigates to `/g/other-group/teams`
- **THEN** the user is redirected

#### Scenario: Non-existent slug is handled gracefully
- **WHEN** a user navigates to `/g/no-such-slug/teams`
- **THEN** the user is redirected or shown a 404

#### Scenario: System admin can access any group's pages
- **WHEN** a user with role :admin navigates to `/g/:any_slug/teams`
